...
class NotificationAgent:
    ...
    async def notify_email(self, session: aiohttp.ClientSession, alert: dict) -> None:
        ...
        payload = {
            "personalizations": [{"to": [{"email": os.getenv("ALERT_EMAIL_TO", "admin@example.com")}]}],
            "from": {"email": os.getenv("ALERT_EMAIL_FROM", "system@example.com")},
            "subject": f"[{alert['severity']}] Anomaly Alert",
            "content": [{"type": "text/plain", "value": f"Anomaly score: {alert['score']}\nEvent ID: {alert['event_id']}"}]
        }
        async with session.post("https://api.sendgrid.com/v3/mail/send", headers=headers, json=payload) as resp:
            if resp.status >= 400:
