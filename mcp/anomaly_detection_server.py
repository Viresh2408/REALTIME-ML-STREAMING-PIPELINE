...
DB_URL = os.environ.get("DATABASE_URL", "postgresql://anomaly_admin:StrongPass123!@timescaledb:5432/anomaly_db")
...
async def get_event_by_id(event_id: str) -> Optional[Dict[str, Any]]:
    ...
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        record = await conn.fetchrow("SELECT * FROM anomaly_events WHERE event_id = $1", uuid.UUID(event_id))
...
async def label_event(event_id: str, label: str, analyst_id: str) -> Dict[str, Any]:
    ...
    async with pool.acquire() as conn:
        record = await conn.fetchrow(
            ...
            label, analyst_id, uuid.UUID(event_id)
        )
        return dict(record) if record else {"error": "Event not found"}

@server.tool()
async def get_anomaly_rate(bucket: str = "1h", source_id: Optional[str] = None) -> List[Dict[str, Any]]:
...
@server.tool()
async def query_anomalies_by_source(source_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
...
@server.tool()
async def acknowledge_alert(alert_id: str, analyst_id: str, note: Optional[str] = None) -> Dict[str, Any]:
    ...
    async with pool.acquire() as conn:
        record = await conn.fetchrow(
            ...
            analyst_id, note, uuid.UUID(alert_id)
        )
