...
async def create_annotation(text: str, tags: list[str]) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        payload = {"text": text, "tags": tags, "time": int(datetime.now().timestamp() * 1000)}
        response = await client.post(
            f"{GRAFANA_URL}/api/annotations", json=payload, headers=get_grafana_headers()
        )
...
async def get_dashboard_snapshot(dashboard_uid: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        payload = {"dashboard": {"uid": dashboard_uid}, "expires": 3600}
        response = await client.post(
            f"{GRAFANA_URL}/api/snapshots", json=payload, headers=get_grafana_headers()
        )
        response.raise_for_status()
        return response.json()
